package com.combatreforged.metis.builder.implementation.util;
...
import com.combatreforged.metis.builder.extension.world.effect.MobEffectExtension;
import com.combatreforged.metis.builder.extension.world.inventory.MenuTypeExtension;
...
import static com.combatreforged.metis.builder.implementation.util.ObjectMappings.*;
    ...
    public <T extends Namespaced> T getByIdentifier(final Identifier identifier, Class<T> type) {
        ...
        try {
            if (EntityType.class.isAssignableFrom(type) && Registry.ENTITY_TYPE.getOptional(loc).isPresent()) {
                ...
                return SOUNDS.inverse().containsKey(mcType.toString().toLowerCase().replace("_", "."))
                        ? (T) SOUNDS.inverse().get(mcType.toString().toLowerCase().replace("_", "."))
